from typing import List, Dict
import pandas as pd
from ...base_strategy import BaseStrategy
from ...signals import OptionSignal, Action, OptionType, CreditDebit


class BullCallSpreadStrategy(BaseStrategy):
    """
    Bull Call Spread Strategy
    
    Bullish strategy using call options. Buy lower strike call, sell higher strike call.
    Limited profit and limited risk. Profits when stock rises moderately.
    
    Parameters:
        bullish_signal_threshold (float): RSI threshold for bullish signal (default: 40)
        support_bounce_factor (float): Factor above support for entry (default: 1.02)
        spread_width (float): Width between strikes as % of price (default: 0.05)
        short_delta (float): Target delta for short call (default: 0.30)
        days_to_expiration (int): Days until option expiration (default: 30)
        max_cost (float): Maximum debit as % of spread width (default: 0.60)
    
    Required DataFrame columns:
        - close: Closing prices
        - low: Low prices (for support calculation)
    
    Optional DataFrame attributes:
        - ticker: Stock ticker symbol
    """
    
    def __init__(self, bullish_signal_threshold: float = 40, support_bounce_factor: float = 1.02,
                 spread_width: float = 0.05, short_delta: float = 0.30, 
                 days_to_expiration: int = 30, max_cost: float = 0.60):
        params = {
            "bullish_signal_threshold": bullish_signal_threshold,
            "support_bounce_factor": support_bounce_factor,
            "spread_width": spread_width,
            "short_delta": short_delta,
            "days_to_expiration": days_to_expiration,
            "max_cost": max_cost
        }
        super().__init__(
            name="Bull Call Spread Strategy",
            params=params,
            is_option_trade=True
        )
    
    def _calculate_rsi(self, prices: pd.Series, window: int = 14) -> pd.Series:
        delta = prices.diff()
        gain = (delta.where(delta > 0, 0)).rolling(window=window).mean()
        loss = (-delta.where(delta < 0, 0)).rolling(window=window).mean()
        rs = gain / loss
        return 100 - (100 / (1 + rs))
    
    def generate_signals(self, df: pd.DataFrame) -> List[OptionSignal]:
        signals = []
        
        # Normalize column names to lowercase
        df = self.normalize_dataframe(df)
        
        required_cols = ['close', 'low']
        if not all(col in df.columns for col in required_cols):
            raise ValueError(f"DataFrame must contain columns: {required_cols}")
        
        df_copy = df.copy()
        bullish_threshold = self.params["bullish_signal_threshold"]
        support_bounce_factor = self.params["support_bounce_factor"]
        spread_width = self.params["spread_width"]
        short_delta = self.params["short_delta"]
        days_to_expiration = self.params["days_to_expiration"]
        max_cost = self.params["max_cost"]
        
        df_copy['rsi'] = self._calculate_rsi(df_copy['close'])
        df_copy['support'] = df_copy['low'].rolling(window=20).min()
        
        ticker = getattr(df, 'ticker', 'UNKNOWN')
        
        for idx, row in df_copy.iterrows():
            if pd.isna(row['rsi']) or pd.isna(row['support']):
                continue
                
            # Bullish signal: RSI recovering from oversold and price bouncing off support
            if (row['rsi'] > bullish_threshold and 
                row['close'] >= row['support'] * support_bounce_factor):
                
                current_price = row['close']
                spread_points = current_price * spread_width
                
                # Strike selection
                long_call_strike = current_price  # ATM or slightly ITM
                short_call_strike = long_call_strike + spread_points
                
                # Estimate costs
                long_call_cost = spread_points * 0.7  # ITM call costs more
                short_call_credit = spread_points * 0.3  # OTM call worth less
                net_debit = long_call_cost - short_call_credit
                
                # Only enter if cost is reasonable
                if net_debit <= spread_points * max_cost:
                    from datetime import datetime, timedelta
                    expiration_date = (datetime.now() + timedelta(days=days_to_expiration)).strftime("%Y-%m-%d")
                    
                    signals.extend([
                        # Buy lower strike call
                        OptionSignal(
                            ticker=ticker,
                            strike=round(long_call_strike, 2),
                            option_type=OptionType.CALL,
                            price=long_call_cost,
                            action=Action.BUY,
                            expiration=expiration_date,
                            credit_debit=CreditDebit.DEBIT
                        ),
                        # Sell higher strike call
                        OptionSignal(
                            ticker=ticker,
                            strike=round(short_call_strike, 2),
                            option_type=OptionType.CALL,
                            price=short_call_credit,
                            action=Action.SELL,
                            expiration=expiration_date,
                            credit_debit=CreditDebit.CREDIT
                        )
                    ])
        
        return signals
    
    def get_param_descriptions(self) -> Dict[str, str]:
        return {
            "bullish_signal_threshold": "RSI level above which to consider bullish (30-50)",
            "support_bounce_factor": "Multiple of support level for entry confirmation",
            "spread_width": "Width between call strikes as percentage of stock price",
            "short_delta": "Target delta for short call (determines OTM distance)",
            "days_to_expiration": "Days until expiration (30-45 optimal for spreads)",
            "max_cost": "Maximum debit as percentage of spread width (0.5-0.7)"
        }
    
    def get_required_columns(self) -> List[str]:
        return ["close", "low"]